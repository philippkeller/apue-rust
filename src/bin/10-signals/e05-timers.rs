...
#[derive(Clone, Copy)]
struct Timeout { function:fn(), timeout_s:u32 }

static mut SIGNALS: [Timeout;16] = [Timeout{function:dummy, timeout_s:999999};16];
...
fn sig_alrm(_:c_int) {
    unsafe {
        for (i, signal) in SIGNALS.iter().enumerate() {
            ...
            SIGNALS[i] = Timeout{function:signal.function, timeout_s:signal.timeout_s - LAST_TIMEOUT_S };
        }
        let f = SIGNALS[0].function;
        // "remove" first element and put a dummy element at the end.
        // There are probably nicer ways to do that..
        SIGNALS[0] = Timeout{function:dummy, timeout_s:999999};
...
fn set_timeout(callback: fn(), timeout_s:u32) {
    unsafe {
        SIGNALS[15] = Timeout{function:callback, timeout_s:timeout_s};
        SIGNALS.sort_by(|a, b| a.timeout_s.cmp(&b.timeout_s));
