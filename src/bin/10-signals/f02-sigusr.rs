...
extern "C" fn sig_usr(signo: c_int) {
    match signo {
        ...
        _ => { panic!(format!("received signal {}", signo)); },
    }
}

fn main() {
    unsafe {
        if signal(SIGUSR1, sig_usr as usize) == SIG_ERR { panic!("can't catch SIGUSR1"); }
        if signal(SIGUSR2, sig_usr as usize) == SIG_ERR { panic!("can't catch SIGUSR2"); }
        loop {
            pause();
        }
    }
