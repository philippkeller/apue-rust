...
///
/// My approach: Parse the header files which contain the `PC` and `SC` definition (binding to an int)
/// and print those via sysconf or pathconf. The header file differ for 
...
fn pr_conf(key: &str, path: &str, name:i32) {
	unsafe {
	    ...
	    let res = match &key[0..4] {
	    	...
	    	_ => panic!("key needs to start either with _PC_ or _SC_ but was {:?}", key),
	    };
	    if let Some(val) = res.to_option() {
	    	...
	    	let e = errno();
	    	let _ = match e.0 {
	    	    EINVAL => {puts(cstr!(" (not supported)"));},
	    	    0 => {puts(cstr!(" (no limit)"));},
